...
class EventAnalyticsRepository {
    ...
    ): Result<EventAnalyticsListResponse> =
        safeApiCall { api.apiV1EventsEventsAnalyticsRetrieve(eventId, endDate, page, pageSize, startDate) }

    suspend fun getEventAnalyticsForDate(date: String, eventId: Int): Result<EventAnalyticsDetailResponse> =
        safeApiCall { api.apiV1EventsEventsAnalyticsRetrieve2(date, eventId) }

    suspend fun getEventAnalyticsSummary(eventId: Int, days: Int? = null): Result<EventAnalyticsSummaryResponse> =
