...
class ReactionsRepository {
    ...
    suspend fun createLike(request: LikeCreateRequest): Result<LikeCreateResponse> =
        safeApiCall { api.apiV1FeedLikesCreate(request) }

    suspend fun deleteLike(likeId: Int): Result<LikeDeleteResponse> =
        safeApiCall { api.apiV1FeedLikesDestroy(likeId) }

    suspend fun getMostLikedContent(
        contentType: String,
        days: Int? = null,
        limit: Int? = null
    ): Result<MostLikedContentResponse> =
        safeApiCall { api.apiV1FeedLikesMostLikedRetrieve(contentType, days, limit) }

    suspend fun getMutualLikes(userId: Int): Result<MutualLikesResponse> =
    ...
    suspend fun getMyLikes(
        ...
        page: Int? = null,
        pageSize: Int? = null
    ): Result<LikeListResponse> =
        safeApiCall { api.apiV1FeedLikesRetrieve(contentType, page, pageSize) }

    suspend fun getObjectLikes(
        contentType: String,
        objectId: Int,
        page: Int? = null,
        pageSize: Int? = null
    ): Result<ObjectLikesResponse> =
        safeApiCall { api.apiV1FeedLikesRetrieve2(contentType, objectId, page, pageSize) }

    suspend fun getLike(likeId: Int): Result<LikeDetailResponse> =
        safeApiCall { api.apiV1FeedLikesRetrieve3(likeId) }

    suspend fun getLikeStatistics(userId: Int? = null): Result<UserLikeStatisticsResponse> =
        safeApiCall { api.apiV1FeedLikesStatisticsRetrieve(userId) }

    suspend fun getUserLikeStatistics(userId: Int, userId2: Int? = null): Result<UserLikeStatisticsResponse> =
